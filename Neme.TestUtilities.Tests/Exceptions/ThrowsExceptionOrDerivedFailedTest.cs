...
namespace Neme.UnitTestUtilities.Tests.Exceptions
{
    ...
    public class ThrowsExceptionOrDerivedFailedTest : ThrowsCertainExceptionFailedTest
    {
        ...
        {
            var e1 = TestUtilities.Catch<ArgumentNullException>(() => { new ThrowsExceptionOrDerivedFailed(null); });
            Assert.IsNotNull(e1);
            Assert.AreEqual("expectedException", e1.ParamName);

            var e2 = TestUtilities.Catch<ArgumentNullException>(() => { new ThrowsExceptionOrDerivedFailed(null, null); });
            Assert.IsNotNull(e2);
        ...
        {
            var e1 = TestUtilities.Catch<ArgumentException>(() => { new ThrowsExceptionOrDerivedFailed(typeof(object)); });
            Assert.IsNotNull(e1);
            Assert.AreEqual("expectedException", e1.ParamName);

            var e2 = TestUtilities.Catch<ArgumentException>(() => { new ThrowsExceptionOrDerivedFailed(typeof(int), null); });
            Assert.IsNotNull(e2);
            Assert.AreEqual("expectedException", e2.ParamName);
        }
        ...
        {
            var e1 = TestUtilities.Catch<ArgumentException>(() => { new ThrowsExceptionOrDerivedFailed(typeof(Exception), new Exception()); });
            Assert.IsNotNull(e1);
            Assert.AreEqual("actualException", e1.ParamName);

            var e2 = TestUtilities.Catch<ArgumentException>(() => { new ThrowsExceptionOrDerivedFailed(typeof(AssertFailedException), new AssertFailedException()); });
            Assert.IsNotNull(e2);
        ...
        {
            var e = TestUtilities.Catch<ArgumentException>(() => { new ThrowsExceptionOrDerivedFailed(typeof(ArgumentException), new ArgumentOutOfRangeException()); });
            Assert.IsNotNull(e);
        }
    }
}
